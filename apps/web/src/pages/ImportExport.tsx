...
export default function ImportExport() {
  ...
  const [existingFolders, setExistingFolders] = useState<
    Awaited<ReturnType<typeof api.vault.list>>['folders']
  >([]);
  ...
  const folderCount = new Set(
    records
      .filter((record) => record.folderPath.length > 0)
      .map((record) => record.folderPath.join('\u001f'))
  ).size;
  ...
  async function processFile(file: File) {
    ...
    try {
      ...
      setFileError(error instanceof Error ? error.message : 'Authwell could not read that file.');
    }
  }
  ...
  async function handlePreview() {
    ...
    try {
      ...
      setSelectedSourceIds(
        new Set(
          parseResult.records.filter((record) => record.importable).map((record) => record.sourceId)
        )
      );
      ...
      <div className="import-page__inner">
        <header className="import-page__header">
          <div>
            ...
            <p>Review every item locally before Authwell encrypts and saves it.</p>
          </div>
          <div className="import-page__privacy">
            <Icon name="shield-lock" size={19} />
            <span>
              <strong>Client-side only</strong>Your CSV is never uploaded.
            </span>
          </div>
        </header>
        ...
        <div className="import-page__layout">
          <Card variant="surface" padding="lg" className="import-card">
            <div className="import-card__heading">
              <span className="import-card__icon">
                <Icon name="upload" size={21} />
              </span>
              <div>
                <h2>Import from LastPass</h2>
                <p>Production-ready CSV import with folders, notes, favourites, and TOTP.</p>
              </div>
            </div>
              ...
              <section className="import-select" aria-labelledby="lastpass-file-label">
                <div className="import-provider">
                  ...
                  <span className="import-status import-status--ready">
                    <Icon name="circle-check" size={14} /> Ready
                  </span>
                </div>
                ...
                >
                  <span className="import-dropzone__icon">
                    <Icon name={fileName ? 'file-description' : 'upload'} size={24} />
                  </span>
                  <div>
                    <strong id="lastpass-file-label">
                      {fileName || 'Choose your LastPass export'}
                    </strong>
                    <span>
                      {fileName
                        ? `${records.length} item${records.length === 1 ? '' : 's'} found`
                        : 'Drop a .csv file here or browse · up to 20 MB'}
                    </span>
                  </div>
                  <input
                    ref={fileInputRef}
                ...
                {!fileError && parseResult && (
                  <div className="import-message import-message--success" role="status">
                    ...
                    <span>
                      LastPass format verified. Password values stay hidden during review.
                    </span>
                  </div>
                )}
                ...
                {fileIssues.length > 1 && (
                  <details className="import-issues">
                    <summary>Show {fileIssues.length} file messages</summary>
                    <ul>
                ...
                <div className="import-actions">
                  <Button
                    ...
                    disabled={!parseResult || Boolean(fileError)}
              ...
              <section className="import-review" aria-label="Review LastPass import">
                ...
                {legacyRepairs.length > 0 && (
                  <div className="import-message import-message--warning" role="status">
                    ...
                    <span>
                      <strong>
                        {legacyRepairs.length} legacy secure-note{' '}
                        {legacyRepairs.length === 1 ? 'match' : 'matches'} found.
                      </strong>{' '}
                      For each selected match, Authwell will import the recovered note first, then
                      move its broken <code>http://sn</code> login copy to Trash.
                    </span>
                  </div>
                )}
                ...
                <div className="import-review__controls">
                  <label className="import-search">
                    ...
                    <input
                      ...
                      placeholder="Find an item or folder"
                      aria-label="Find an item or folder"
                    />
                  </label>
                  <Select
                    label="Duplicates"
                ...
                <div className="import-review__meta">
                  ...
                  <button type="button" onClick={() => setSelectedSourceIds(new Set())}>
                    Clear
                  </button>
                </div>
                ...
                <div className="import-table-wrap">
                  <table className="import-table">
                    <thead>
                      <tr>
                        <th className="import-table__check">
                    ...
                    <tbody>
                      {filteredRecords.map((record) => {
                        ...
                        return (
                          <tr
                            key={record.sourceId}
                            className={!record.importable ? 'import-table__row--invalid' : ''}
                          >
                            <td className="import-table__check">
                              <input
                                type="checkbox"
                                checked={selectedSourceIds.has(record.sourceId)}
                                disabled={
                                  !record.importable || (duplicate && duplicateStrategy === 'skip')
                                }
                                onChange={() => toggleRecord(record)}
                                aria-label={`Import ${record.item.name}`}
                              />
                            </td>
                            <td>
                              <div className="import-item">
                                <SiteFavicon
                                  sources={getEntryFaviconSources(record.item)}
                                  fallbackIcon={record.item.type === 'note' ? 'note' : 'key'}
                                  size={20}
                                />
                                <span>
                                  <strong>{record.item.name}</strong>
                                  <small>
                                    {recordSubtitle(record)} · row {record.sourceRow}
                                  </small>
                                </span>
                              </div>
                            </td>
                            <td>
                              <span className="import-folder">
                            ...
                            <td>
                              {!record.importable ? (
                                ...
                                <span className="import-status import-status--warning">
                                  Duplicate
                                </span>
                              ) : firstIssue ? (
                                <span className="import-status import-status--warning">
                                ...
                                <span className="import-status import-status--ready">Ready</span>
                              )}
                              {firstIssue && (
                                <small className="import-table__issue">{firstIssue.message}</small>
                              )}
                            </td>
                          </tr>
                        );
                ...
                {records.filter((record) => recordMatches(record, reviewQuery.trim())).length >
                  MAX_RENDERED_RECORDS && (
                  <p className="import-review__limit">
                    Showing the first {MAX_RENDERED_RECORDS} matches. All selected items will still
                    import.
                  </p>
                )}
                ...
                <div className="import-actions import-actions--split">
                  ...
                  >
                    <Icon name="lock" size={17} /> Encrypt &amp; import {selectedImportCount}
                  </Button>
                </div>
              </section>
            )}
            ...
            {importStep === 'importing' && (
              <section className="import-progress" aria-live="polite">
                ...
                <p>
                  Folders are prepared first, then each item is encrypted before it leaves this
                  device.
                </p>
                <div
            ...
            {importStep === 'done' && importResult && (
              <section className="import-done">
                <span className="import-done__icon">
                  <Icon
                    name={
                      importResult.failures.length > 0 || importResult.cleanupFailures.length > 0
                        ? 'alert-triangle'
                        : 'circle-check'
                    }
                    size={30}
                ...
                <h3>
                  {importResult.failures.length > 0 || importResult.cleanupFailures.length > 0
                    ...
                    : 'Your passwords are in Authwell'}
                </h3>
                <p>
                  {importResult.importedCount} item{importResult.importedCount === 1 ? '' : 's'}{' '}
                  imported · {importResult.createdFolders.length} folder
                  {importResult.createdFolders.length === 1 ? '' : 's'} created
                  {importResult.duplicateSkippedCount > 0
                    ? ` · ${importResult.duplicateSkippedCount} duplicate${importResult.duplicateSkippedCount === 1 ? '' : 's'} skipped`
                    : ''}
                  {importResult.legacyRepairedCount > 0
                    ? ` · ${importResult.legacyRepairedCount} broken secure-note ${importResult.legacyRepairedCount === 1 ? 'copy' : 'copies'} moved to Trash`
                    : ''}
                  .
                </p>
                {importResult.failures.length > 0 && (
                  <div className="import-failures" role="alert">
                    <strong>
                      {importResult.failures.length} item
                      {importResult.failures.length === 1 ? '' : 's'} could not be imported
                    </strong>
                    <ul>
                      {importResult.failures.map((failure) => (
                        <li key={failure.sourceId}>
                          <span>
                            {failure.itemName} · row {failure.sourceRow}
                          </span>
                ...
                {importResult.cleanupFailures.length > 0 && (
                  <div className="import-failures" role="alert">
                    <strong>
                      {importResult.cleanupFailures.length} broken{' '}
                      {importResult.cleanupFailures.length === 1 ? 'copy remains' : 'copies remain'}
                    </strong>
                    <ul>
                      {importResult.cleanupFailures.map((failure) => (
                        <li key={failure.sourceId}>
                          <span>
                            {failure.itemName} · row {failure.sourceRow}
                          </span>
          ...
          <aside className="import-sidebar">
            <Card variant="surface" padding="lg">
              <div className="import-card__heading">
                ...
                <div>
                  <h2>Export from Authwell</h2>
                  <p>Download a Bitwarden-compatible CSV.</p>
                </div>
              </div>
              <div className="import-message import-message--warning">
                <Icon name="alert-triangle" size={17} />
                <span>
                  The export contains plaintext passwords. Delete it as soon as you finish moving
                  your data.
                </span>
              </div>
              <Button
            ...
            <Card variant="surface" padding="lg" className="import-guide">
              ...
              <ol>
                <li>
                  <span>1</span>
                  <p>
                    Open LastPass and choose <strong>Advanced Options</strong>.
                  </p>
                </li>
                <li>
                  <span>2</span>
                  <p>
                    Select <strong>Export</strong>, then download the LastPass CSV.
                  </p>
                </li>
                <li>
                  <span>3</span>
                  <p>Return here, review the items, then delete the CSV.</p>
                </li>
              </ol>
              <p className="import-guide__future">
                <Icon name="settings" size={16} /> The provider adapter is ready for more password
                managers in v2.
              </p>
            </Card>
          </aside>
