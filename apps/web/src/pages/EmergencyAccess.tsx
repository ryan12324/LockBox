...
function statusBadge(status: string) {
  const map: Record<string, { bg: string; text: string; label: string }> = {
    ...
    confirmed: { bg: 'bg-blue-500/20', text: 'text-blue-300', label: 'Confirmed' },
    waiting: { bg: 'bg-orange-500/20', text: 'text-orange-300', label: 'Waiting' },
    approved: { bg: 'bg-green-500/20', text: 'text-green-300', label: 'Approved' },
    rejected: { bg: 'bg-red-500/20', text: 'text-red-300', label: 'Rejected' },
    revoked: { bg: 'bg-white/[0.08]', text: 'text-white/50', label: 'Revoked' },
  };
  const s = map[status] ?? { bg: 'bg-white/[0.08]', text: 'text-white/50', label: status };
  return (
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6">
        ...
        <h2 className="text-xl font-medium text-white mb-2">Loading Emergency Access</h2>
        <p className="text-white/60">Fetching your emergency contacts…</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold text-white mb-1">🛡️ Emergency Access</h1>
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-6 bg-white/[0.06] p-1 rounded-lg">
          {[
            ...
            >
              {tab.label}
              {tab.count > 0 && (
                <span className="ml-2 px-1.5 py-0.5 rounded-full bg-white/[0.12] text-xs">
                  {tab.count}
                </span>
              )}
            </button>
        ...
        {activeTab === 'trusted' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-white">Your Trusted Contacts</h2>
              <button
            ...
            {showAdd && (
              <div className="mb-6 p-4 bg-white/[0.06] border border-white/[0.1] rounded-lg">
                <h3 className="text-sm font-semibold text-white mb-3">Add Trusted Contact</h3>
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-white/[0.06] border border-white/[0.12] rounded-lg text-sm text-white placeholder-white/40"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-white/60 mb-1">Wait period</label>
                    <select
                      value={addWait}
                      onChange={(e) => setAddWait(Number(e.target.value))}
                      className="w-full px-3 py-2 bg-white/[0.06] border border-white/[0.12] rounded-lg text-sm text-white"
                    >
                      {WAIT_PERIODS.map((p) => (
            ...
            {grants.length === 0 ? (
              ...
              <div className="space-y-3">
                {grants.map((grant) => (
                  <div
                    key={grant.id}
                    className="p-4 bg-white/[0.06] border border-white/[0.1] rounded-lg flex items-center justify-between"
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-white font-medium text-sm">{grant.granteeEmail}</span>
        ...
        {activeTab === 'requests' && (
          <div>
            <h2 className="text-lg font-semibold text-white mb-4">Grants Available to You</h2>
            ...
            {requests.length === 0 ? (
              ...
              <div className="space-y-3">
                {requests.map((grant) => (
                  <div
                    key={grant.id}
                    className="p-4 bg-white/[0.06] border border-white/[0.1] rounded-lg"
                  >
                      ...
                      <div className="flex gap-2">
                        ...
                        {grant.status === 'confirmed' && (
                          <button
                            ...
                            className="px-3 py-1.5 text-xs bg-orange-600 text-white rounded-lg hover:bg-orange-500 transition-colors"
                          >
                            Request Access
                          </button>
                        )}
                        {grant.status === 'approved' && (
                          <button className="px-3 py-1.5 text-xs bg-green-600 text-white rounded-lg hover:bg-green-500 transition-colors">
                            View Vault
                          </button>
                        )}
                      </div>
