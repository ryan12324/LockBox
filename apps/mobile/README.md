...
| Biometric unlock | BiometricPrompt + Keystore | LocalAuthentication + Keychain access control |
...
Biometric vault unlock is opt-in under **Authwell Settings → Native biometric unlock**. Enrollment binds the current account's 64-byte vault key to Face ID or Touch ID and adds a biometric action to the lock screen; switching accounts or servers cannot reuse the prior key. Signing out clears the native AutoFill index, biometric enrollment, and offline database before ending the local session.
