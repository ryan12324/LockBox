...
function parseApiOrigin(value: unknown, webOrigin: string): string {
  ...
  if (url.protocol !== 'https:' && !(url.protocol === 'http:' && isLoopback(url.hostname))) {
    throw new LockboxDiscoveryError(
      'discovery-invalid',
      'The discovered Lockbox API must use HTTPS.'
    );
  }
  if (
    url.username ||
...
async function readDiscoveryDocument(
  ...
  if (response.status === 404) {
    throw new LockboxDiscoveryError(
      'discovery-not-found',
      'This server does not publish Lockbox extension settings.'
    );
  }
  if (!response.ok) {
    throw new LockboxDiscoveryError(
      'discovery-unavailable',
      `The web vault could not provide extension settings (HTTP ${response.status}).`
    );
  }
  assertJsonResponse(
    response,
  ...
  try {
    ...
    throw new LockboxDiscoveryError(
      'discovery-invalid',
      'The web vault returned malformed extension settings. Redeploy it and try again.'
    );
  }
  ...
  if (!isRecord(body) || body.product !== LOCKBOX_PRODUCT) {
    throw new LockboxDiscoveryError(
      'discovery-invalid',
      'That address is not a Lockbox web vault.'
    );
  }
  if (body.protocolVersion !== LOCKBOX_PROTOCOL_VERSION) {
    throw new LockboxDiscoveryError(
      'unsupported-protocol',
      'This Lockbox web vault uses an unsupported extension protocol. Update the web vault and extension together.'
    );
  }
...
export async function discoverLockboxServer(
  ...
  const webUrl = parseTrustedOrigin(input.trim());
  ...
  try {
    ...
    // Compatibility for users who enter a Worker URL directly. It is accepted
    // only when the origin positively identifies as a compatible Lockbox API.
