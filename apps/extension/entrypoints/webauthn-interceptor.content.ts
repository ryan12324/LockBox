/**
 * WebAuthn interceptor — registered MAIN-world content script.
 *
  ...
  main() {
    ...
    navigator.credentials.create = function (
      ...
      return waitForResponse(requestId, timeout).then((resp) => {
        if (resp.error) {
          throw new DOMException(resp.error, resp.errorName ?? 'NotAllowedError');
        }
        if (resp.fallback) return origCreate(options);
    ...
    navigator.credentials.get = function (
      ...
      return waitForResponse(requestId, timeout).then((resp) => {
        ...
        if (resp.fallback) return origGet(options);
        if (resp.credential) return deserializeCredential(resp.credential);
