...
const DEFAULT_LEGITIMATE_DOMAINS = [
    'google.com',
    'facebook.com',
    'amazon.com',
    'apple.com',
 ...
 */
function extractRawHostname(raw) {
    const trimmed = raw.trim();
    // Strip protocol
...
export class PhishingDetector {
    ...
    analyzeUrl(url) {
        ...
        if (!homoglyphCheck.passed)
            totalWeight += WEIGHTS.homoglyph;
        ...
        if (!keywordCheck.passed)
            totalWeight += WEIGHTS.keywordStuffing;
        // (h) Long URL check
        const longUrlCheck = this.checkLongUrl(fullUrl);
        checks.push(longUrlCheck);
        if (!longUrlCheck.passed)
            totalWeight += WEIGHTS.longUrl;
        // Compute final score
    ...
    checkHomoglyphs(hostname) {
        ...
        const resembles = this.legitimateDomains.some((d) => normDomain === d || normalised.includes(d.split('.')[0]));
        if (resembles) {
            return {
                name: 'homoglyph',
                passed: false,
     ...
     */
    checkTyposquat(hostname, skipIfHomoglyph) {
        if (skipIfHomoglyph) {
            return {
                name: 'typosquat',
                passed: true,
    ...
    checkSuspiciousTld(hostname) {
        ...
        if (dot === -1)
            return { name: 'suspicious-tld', passed: true };
