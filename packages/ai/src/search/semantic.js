 ...
 */
function keywordFallbackScore(query, itemText) {
 ...
 */
function hasExactMatch(query, itemText) {
    const queryWords = query
        .toLowerCase()
        .split(/\s+/)
        .filter((w) => w.length > 0);
...
export class SemanticSearch {
    ...
    async index(items) {
        ...
        if ('buildVocabulary' in this.provider) {
            this.provider.buildVocabulary(texts);
        }
