...
export function suggestTags(item: VaultItem): string[] {
  ...
  for (const [kw, kwCats] of Object.entries(KEYWORD_MAP)) {
    if (nameLower.includes(kw)) kwCats.forEach((c) => tags.add(c));
  }
...
export function suggestFolder(item: VaultItem, folders: Folder[]): string | null {
  const tags = suggestTags(item);
  if (tags.length === 0 || folders.length === 0) return null;
  ...
  for (const folder of folders) {
    ...
    for (const tag of tags) {
      if (folderLower.includes(tag)) score++;
    }
    if (score > bestScore) {
      bestScore = score;
      bestMatch = folder.id;
...
export function detectDuplicates(items: VaultItem[]): DuplicateGroup[] {
  const groups: DuplicateGroup[] = [];
  const loginItems = items.filter((i): i is LoginItem => i.type === 'login');

  // Same URI domain
  const byDomain = new Map<string, LoginItem[]>();
  ...
  for (const [, domainItems] of byDomain) {
    if (domainItems.length >= 2) {
      ...
      if (unique.length >= 2) groups.push({ items: unique, reason: 'same-uri' });
    }
  }

  // Same username (across different domains)
  const byUsername = new Map<string, LoginItem[]>();
  for (const item of loginItems) {
  ...
  for (const [, usernameItems] of byUsername) {
    if (usernameItems.length >= 2) {
      ...
      if (domains.size >= 2) groups.push({ items: usernameItems, reason: 'same-credentials' });
    }
  }
