 ...
 */
export async function encryptUserKey(userKey, masterKey) {
 ...
 */
export async function decryptUserKey(encryptedUserKey, masterKey) {
    const aesKey = masterKey.slice(0, 32);
 ...
 */
export async function makeAuthHash(masterKey, password) {
    const keyMaterial = await crypto.subtle.importKey('raw', toUtf8(toBase64(masterKey)), { name: 'PBKDF2' }, false, ['deriveBits']);
    const derivedBits = await crypto.subtle.deriveBits({
        name: 'PBKDF2',
        hash: 'SHA-256',
        salt: toUtf8(password),
        iterations: 1,
    }, keyMaterial, 256);
