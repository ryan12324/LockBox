...
function mergeRules(rules, partial, source) {
    if (partial.minLength !== undefined)
        rules.minLength = partial.minLength;
    if (partial.maxLength !== undefined)
        rules.maxLength = partial.maxLength;
    if (partial.requireUppercase)
        rules.requireUppercase = true;
    if (partial.requireLowercase)
        rules.requireLowercase = true;
    if (partial.requireDigit)
        rules.requireDigit = true;
    if (partial.requireSpecial)
...
function parsePattern(pattern) {
    ...
    if (lenMatch) {
        ...
        if (!isNaN(min) && min > 0)
            partial.minLength = min;
        if (lenMatch[2]) {
            const max = parseInt(lenMatch[2], 10);
            if (!isNaN(max) && max > 0)
                partial.maxLength = max;
        }
    }
    return partial;
}
...
function parseText(text) {
    ...
    // --- Length requirements ---
    const minLen = extractNumber(lower, /at\s+least\s+(\d+)\s*(?:char|letter|digit)/, /minimum\s+(?:of\s+)?(\d+)\s*(?:char|letter|digit)/, /min(?:imum)?\s*(?:length)?[:\s]+(\d+)/, /(\d+)\s*(?:char|letter)\w*\s+(?:or\s+)?(?:more|min)/, /at\s+least\s+(\d+)/);
    if (minLen !== null)
        partial.minLength = minLen;
    const maxLen = extractNumber(lower, /(?:no\s+more\s+than|at\s+most|maximum\s+(?:of\s+)?|max(?:imum)?\s*(?:length)?[:\s]+)(\d+)/, /up\s+to\s+(\d+)\s*char/);
    if (maxLen !== null)
        partial.maxLength = maxLen;
    ...
    if (rangeMatch) {
        ...
        if (!isNaN(lo) && lo > 0)
            partial.minLength = lo;
        if (!isNaN(hi) && hi > 0)
            partial.maxLength = hi;
    }
 ...
 */
export function detectPasswordRules(field) {
...
export function generateCompliant(rules) {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        if (upperPool.length > 0)
            chars.push(upperPool[getRandomIndex(upperPool.length)]);
        if (lowerPool.length > 0)
            chars.push(lowerPool[getRandomIndex(lowerPool.length)]);
        if (digitPool.length > 0)
            chars.push(digitPool[getRandomIndex(digitPool.length)]);
        if (specialPool.length > 0)
            chars.push(specialPool[getRandomIndex(specialPool.length)]);
 ...
 */
export function smartGenerate(field) {
...
function verifyCompliance(password, rules) {
    ...
    if (rules.forbiddenChars) {
        const forbidden = new Set(rules.forbiddenChars.split(''));
        if (password.split('').some((c) => forbidden.has(c)))
            return false;
    }
    return true;
}
